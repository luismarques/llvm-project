...
#include "Plugins/Process/Utility/RegisterContextLinux_riscv.h"
...
enum pt_watch_style { pt_watch_style_riscv32, pt_watch_style_riscv64 };
struct riscv32_watch_regs {
  uint32_t watchlo[8];
  uint16_t watchhi[8];
...
struct pt_watch_regs {
  ...
  union {
    struct riscv32_watch_regs riscv32;
    struct riscv64_watch_regs riscv64;
  };
};
...
#define REG_CONTEXT_SIZE 32

// NativeRegisterContextLinux_riscv members.

static RegisterInfoInterface *
CreateRegisterInfoInterface(const ArchSpec &target_arch) {
  if (target_arch.GetMachine() == llvm::Triple::riscv64) {
    return new RegisterContextLinux_riscv(
        target_arch);
  }
  llvm_unreachable("LUIS");
}
...
NativeRegisterContextLinux_riscv::NativeRegisterContextLinux_riscv(
  ...
  case llvm::Triple::riscv64:
    m_reg_info.num_registers = k_num_registers_riscv;
    m_reg_info.num_gpr_registers = k_num_gpr_registers_riscv;
    m_reg_info.num_fpr_registers = k_num_fpr_registers_riscv;
    m_reg_info.last_gpr = k_last_gpr_riscv;
    m_reg_info.first_fpr = k_first_fpr_riscv;
    m_reg_info.last_fpr = k_last_fpr_riscv;
    break;
  default:
...
uint32_t NativeRegisterContextLinux_riscv::GetRegisterSetCount() const {
  ...
  case llvm::Triple::riscv64: {
    const auto context = static_cast<const RegisterContextLinux_riscv &>
...
lldb::addr_t NativeRegisterContextLinux_riscv::GetPCfromBreakpointLocation(
  ...
  const RegisterInfo *const pc_info_p = GetRegisterInfoAtIndex(gpr_pc_riscv);
  ...
  if (error.Success()) {
    ...
    const RegisterInfo *const cause_info_p =
        GetRegisterInfoAtIndex(gpr_cause_riscv);
...
lldb_private::Status NativeRegisterContextLinux_riscv::WriteRegister(
...
Status NativeRegisterContextLinux_riscv::ReadAllRegisterValues(
  ...
  ::memcpy(dst, &m_msa, sizeof(MSA_linux_riscv));
...
Status NativeRegisterContextLinux_riscv::WriteAllRegisterValues(
  ...
  if (!data_sp) {
    error.SetErrorStringWithFormat(
        "NativeRegisterContextLinux_riscv::%s invalid data_sp provided",
        __FUNCTION__);
    return error;
  }
  ...
  if (data_sp->GetByteSize() != REG_CONTEXT_SIZE) {
    error.SetErrorStringWithFormat(
        "NativeRegisterContextLinux_riscv::%s data_sp contained mismatched "
  ...
  if (src == nullptr) {
    error.SetErrorStringWithFormat("NativeRegisterContextLinux_riscv::%s "
  ...
  ::memcpy(&m_msa, src, sizeof(MSA_linux_riscv));

  error = WriteGPR();
  if (!error.Success()) {
    error.SetErrorStringWithFormat(
        "NativeRegisterContextLinux_riscv::%s WriteGPR() failed",
        __FUNCTION__);
    return error;
  }
  ...
  if (!error.Success()) {
    error.SetErrorStringWithFormat(
        "NativeRegisterContextLinux_riscv::%s WriteCP1() failed",
        __FUNCTION__);
    return error;
  }
...
uint8_t *
NativeRegisterContextLinux_riscv::ReturnFPOffset(uint8_t reg_index,
...
bool NativeRegisterContextLinux_riscv::IsFPR(uint32_t reg_index) const {
...
static uint32_t GetWatchHi(struct pt_watch_regs *regs, uint32_t index) {
  ...
  if (regs->style == pt_watch_style_riscv32)
    return regs->riscv32.watchhi[index];
  else if (regs->style == pt_watch_style_riscv64)
    return regs->riscv64.watchhi[index];
...
static void SetWatchHi(struct pt_watch_regs *regs, uint32_t index,
  ...
  Log *log(ProcessPOSIXLog::GetLogIfAllCategoriesSet(POSIX_LOG_WATCHPOINTS));
  if (regs->style == pt_watch_style_riscv32)
    regs->riscv32.watchhi[index] = value;
  else if (regs->style == pt_watch_style_riscv64)
    regs->riscv64.watchhi[index] = value;
  LLDB_LOG(log, "Invalid watch register style");
  return;
}
...
static lldb::addr_t GetWatchLo(struct pt_watch_regs *regs, uint32_t index) {
  Log *log(ProcessPOSIXLog::GetLogIfAllCategoriesSet(POSIX_LOG_WATCHPOINTS));
  if (regs->style == pt_watch_style_riscv32)
    return regs->riscv32.watchlo[index];
  else if (regs->style == pt_watch_style_riscv64)
    return regs->riscv64.watchlo[index];
  LLDB_LOG(log, "Invalid watch register style");
...
static void SetWatchLo(struct pt_watch_regs *regs, uint32_t index,
  ...
  Log *log(ProcessPOSIXLog::GetLogIfAllCategoriesSet(POSIX_LOG_WATCHPOINTS));
  if (regs->style == pt_watch_style_riscv32)
    regs->riscv32.watchlo[index] = (uint32_t)value;
  else if (regs->style == pt_watch_style_riscv64)
    regs->riscv64.watchlo[index] = value;
  else
    LLDB_LOG(log, "Invalid watch register style");
}
...
static uint32_t GetIRWMask(struct pt_watch_regs *regs, uint32_t index) {
  Log *log(ProcessPOSIXLog::GetLogIfAllCategoriesSet(POSIX_LOG_WATCHPOINTS));
  if (regs->style == pt_watch_style_riscv32)
    return regs->riscv32.watch_masks[index] & IRW;
  else if (regs->style == pt_watch_style_riscv64)
    return regs->riscv64.watch_masks[index] & IRW;
  LLDB_LOG(log, "Invalid watch register style");
  return 0;
}
...
Status NativeRegisterContextLinux_riscv::IsWatchpointHit(uint32_t wp_index,
                                                          bool &is_hit) {
...
Status NativeRegisterContextLinux_riscv::GetWatchpointHitIndex(
...
Status NativeRegisterContextLinux_riscv::IsWatchpointVacant(uint32_t wp_index,
                                                             bool &is_vacant) {
  is_vacant = false;
  return Status("MIPS TODO: "
                "NativeRegisterContextLinux_riscv::IsWatchpointVacant not "
                "implemented");
}
...
bool NativeRegisterContextLinux_riscv::ClearHardwareWatchpoint(
  ...
  if (regs.style == pt_watch_style_riscv32) {
    regs.riscv32.watchlo[wp_index] = default_watch_regs.riscv32.watchlo[wp_index];
    regs.riscv32.watchhi[wp_index] = default_watch_regs.riscv32.watchhi[wp_index];
    regs.riscv32.watch_masks[wp_index] =
        default_watch_regs.riscv32.watch_masks[wp_index];
  } else // pt_watch_style_riscv64
  {
    regs.riscv64.watchlo[wp_index] = default_watch_regs.riscv64.watchlo[wp_index];
    regs.riscv64.watchhi[wp_index] = default_watch_regs.riscv64.watchhi[wp_index];
    regs.riscv64.watch_masks[wp_index] =
        default_watch_regs.riscv64.watch_masks[wp_index];
  }
...
Status NativeRegisterContextLinux_riscv::SetHardwareWatchpointWithIndex(
    lldb::addr_t addr, size_t size, uint32_t watch_flags, uint32_t wp_index) {
  Status error;
  error.SetErrorString("MIPS TODO: "
                       "NativeRegisterContextLinux_riscv::"
...
#if 0
  if (reg_info->kinds[lldb::eRegisterKindDWARF] == dwarf_bad_riscv64) {
    EmulatorBaton *emulator_baton = static_cast<EmulatorBaton *>(baton);
...
uint32_t NativeRegisterContextLinux_riscv::NumSupportedHardwareWatchpoints() {
  ...
  if (!num_valid) {
    ...
    switch (regs.style) {
    case pt_watch_style_riscv32:
      num_valid = regs.riscv32.num_valid; // Using num_valid as cache
      return num_valid;
    case pt_watch_style_riscv64:
      num_valid = regs.riscv64.num_valid;
      return num_valid;
    }
    LLDB_LOG(log, "Invalid watch register style");
    return 0;
...
Status
NativeRegisterContextLinux_riscv::ReadRegisterRaw(uint32_t reg_index,
                                                   RegisterValue &value) {
  ...
  if ((offset == ptrace_sr_riscv) || (offset == ptrace_config5_riscv))
...
Status NativeRegisterContextLinux_riscv::WriteRegisterRaw(
...
Status NativeRegisterContextLinux_riscv::Read_SR_Config(uint32_t offset,
                                                         const char *reg_name,
                                                         uint32_t size,
                                                         RegisterValue &value) {
  size_t regs[32];
  ::memset(regs, 0, 32 * 8); // TODO LUIS
  ...
  if (error.Success()) {
    ...
    void *target_address = ((uint8_t *)&regs) + offset +
                           4 * (arch.GetMachine() == llvm::Triple::riscv64);
    value.SetUInt(*(uint32_t *)target_address, size);
  }
  return error;
}

Status NativeRegisterContextLinux_riscv::DoReadWatchPointRegisterValue(
...
Status NativeRegisterContextLinux_riscv::DoWriteWatchPointRegisterValue(
