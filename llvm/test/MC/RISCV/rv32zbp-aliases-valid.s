# RUN: llvm-mc %s -triple=riscv32 -mattr=+experimental-zbp -M no-aliases \
...
# RUN:     | llvm-objdump --no-print-imm-hex -d -r -M no-aliases --mattr=+experimental-zbp - \
# RUN:     | FileCheck -check-prefixes=CHECK-S-OBJ-NOALIAS %s
# RUN: llvm-mc -filetype=obj -triple riscv32 -mattr=+experimental-zbp < %s \
# RUN:     | llvm-objdump --no-print-imm-hex -d -r --mattr=+experimental-zbp - \
# RUN:     | FileCheck -check-prefixes=CHECK-S-OBJ %s
